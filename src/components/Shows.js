...
const Shows = (props) => {
  ...
  useEffect(() => {
    const fetchShows = () => {
      eldenRingApi
        .get(category, { params: { limit: 16, page: page || 0 } })
        .then((response) => {
  ...
  const RenderPages = () => {
    if (pages === 0) {
      ...
      for (let i = 0; i <= pages; i++) {
        ...
        rows.push(
          <a className="link" href={pageLink}>
            {i}
          </a>
        );
      }
      ...
      const renderedPages = rows.map((row) => {
        return <React.Fragment key={row.props.href}> {row} </React.Fragment>;
      });
      return <div style={{ textAlign: "center" }}>{renderedPages}</div>;
    }
  };
